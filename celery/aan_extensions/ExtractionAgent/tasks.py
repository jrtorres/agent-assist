...
@app.task(base=BaseTask.BaseTask, bind=True, soft_time_limit=30)
                        ...
                        with trace.get_tracer(__name__).start_as_current_span(
                            ...
                            if client_id: #must have client_id, otherwise it is a session_start or end
                                ...
                                if (turns_counter != 0):
                                    ...
                                    agent_transcriptions = " . ".join(internal_transcriptions)
                                    user_transcriptions = " . ".join(external_transcriptions)
                                    with trace.get_tracer(__name__).start_as_current_span(
                                        "extraction"):
                                        extracted_entities = process_message(agent_transcript=agent_transcriptions, user_transcript=user_transcriptions) 
