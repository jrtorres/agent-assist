...
from aan_extensions import TranscriptionAgent, ExtractionAgent, CacheAgent, SummaryAgent, NextBestActionAgent, SentimentAgent
            ...
            with trace.get_tracer(__name__).start_as_current_span("dispatch_tasks") as child_span:
                ...
                chained_tasks = chain(
                   ...
                   # NextBestActionAgent.tasks.process_transcript.si(topic,message),
                    ...
                    group(
                         SummaryAgent.tasks.process_transcript.si(topic,message),
                         ExtractionAgent.tasks.process_transcript.si(topic,message),
                         SentimentAgent.tasks.process_transcript.si(topic,message)
                    )
                )
